...
class MVCServer extends BaseServer {
   ...
   * and registers custom Handlebars helpers.
   */
  initViewEngine() {
    this.express.engine(
      "hbs",
      engine({
        defaultLayout: this.config.views_default_layout,
        layoutsDir: this.config.views_layouts_path,
        partialsDir: this.config.views_partials_path,
        extname: "hbs",
        helpers: {
          /**
           * Returns true if a === b.
          ...
          // Additional helpers can be registered here.
        },
      })
    );

    this.express.set("view engine", "hbs");
    this.express.set("views", this.config.views_path);
  ...
  /**
   * Loads a feature router dynamically from the /features directory.
   *
   * @param {string} route - The base route (e.g. "/support").
   * @param {string} name - The feature name (e.g. "support" for supportRouter.js).
   */
  feature(route, name) {
    const filename = path.resolve(`./src/features/${name}/${name}Router.js`);
    const routerClass = require(filename);
    this.router(route, routerClass);
  }
}
